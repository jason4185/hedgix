...
class Hedgix(gl.Contract):
    ...
    def purchase_protection(
        ...
        purchase_datetime = self._message_datetime_text()
        purchase_date = self._date_from_datetime_text(purchase_datetime)
        duration_int = self._positive_u256_to_int(duration_days, "UNSUPPORTED_DURATION")
        coverage_start_date = self._add_days(purchase_date, 1)
        coverage_end_date = self._add_days(coverage_start_date, duration_int - 1)
        terms = self._load_product_terms(
            protected_asset, protection_type, event_level, duration_days
        )
        premium_wei = self._terms_amount_to_u256(terms, "premium_wei")
        payout_wei = self._terms_amount_to_u256(terms, "payout_wei")
        ...
        reference_price_scaled = self._fetch_binance_live_ticker_price_scaled(
            str(terms["binance_settlement_symbol"])
        )
    ...
    @gl.public.write
    def settle_protection_day(self, protection_id: u256, settlement_date: str) -> str:
        self._require_not_paused()
        key = self._protection_key(protection_id)
        protection = self._require_protection(key)
        self._require_settlement_caller(protection)
    ...
    @gl.public.view
    def get_my_dashboard_summary(self) -> str:
        return self._dashboard_summary_for_sender(u256(0), u256(0), False)

    @gl.public.view
    def get_my_dashboard_summary_paginated(self, start: u256, limit: u256) -> str:
        return self._dashboard_summary_for_sender(start, limit, True)
    ...
    def get_dashboard_summary(self) -> str:
        ...
        summary = {
            "owner": self.owner,
            "settlement_operator": self.settlement_operator,
            "registry_url": self.registry_url,
            "registry_version": self.registry_version,
            "paused": self.paused,
            "next_protection_id": str(self.next_protection_id),
            "active_protection_count": str(len(active_ids)),
        }
        summary.update(self._pool_status_fields())
        return self._dumps_json(summary)

    @gl.public.view
    def get_pool_status(self) -> str:
        return self._dumps_json(self._pool_status_fields())
    ...
    @gl.public.view
    def get_active_protection_ids_paginated(self, start: u256, limit: u256) -> str:
        active_ids = self._loads_json_list(self.active_protection_ids)
        sliced_ids = self._slice_ids(active_ids, start, limit)
        start_int = self._u256_to_int(start)
        limit_int = self._u256_to_int(limit)
        return self._dumps_json(
            {
                "start": str(start),
                "limit": str(limit),
                "returned_count": str(len(sliced_ids)),
                "total_count": str(len(active_ids)),
                "has_more": start_int + limit_int < len(active_ids),
                "protection_ids": sliced_ids,
            }
        )

    @gl.public.view
    def get_expected_settlement_date(self, protection_id: u256) -> str:
        protection = self._require_protection(self._protection_key(protection_id))
        return self._expected_settlement_date(protection)

    @gl.public.view
    def get_settlement_readiness(self, protection_id: u256) -> str:
        protection = self._require_protection(self._protection_key(protection_id))
        expected_date = self._expected_settlement_date(protection)
        latest_closed_date = self._latest_closed_daily_date()
        status = str(protection["status"])
        ready = True
        reason = "ready"
        if status != STATUS_ACTIVE:
        ...
        elif self._date_to_ordinal(expected_date) > self._date_to_ordinal(latest_closed_date):
            ready = False
            reason = "daily candle is not closed"
        return self._dumps_json({
            "ready": ready,
            "reason": reason,
            "expected_settlement_date": expected_date,
            "latest_closed_daily_date": latest_closed_date,
            "status": status,
        })
    ...
    def _new_protection_record(
        ...
        return {
            ...
            "reference_price_datetime": reference_price_datetime,
            ...
            "created_at": reference_price_datetime,
        }
    ...
    def _load_product_terms(
        ...
        requested_duration = self._positive_u256_to_int(duration_days, "UNSUPPORTED_DURATION")
        registry_url = self.registry_url
        registry_version = self.registry_version
        ...
        def fetch_selected_terms_text() -> str:
            ...
            if metadata.get("status") != "draft":
                raise gl.vm.UserError("INVALID_REGISTRY_RESPONSE")
            if not self._duration_supported(registry, requested_duration):
                raise gl.vm.UserError("UNSUPPORTED_DURATION")
            ...
            selected_terms = {
                ...
                "duration_days": str(requested_duration),
            }
            if "premium" in level:
    ...
    def _duration_supported(self, registry: typing.Any, duration_days: int) -> bool:
        for duration in registry.get("supported_durations", []):
            try:
                if int(str(duration.get("duration_days", ""))) == duration_days:
                    return True
            except Exception:
                pass
        return False
    ...
    def _parse_selected_terms(
        ...
        if returned_level != requested_level:
            raise gl.vm.UserError("REGISTRY_TERMS_MISMATCH")
        try:
            returned_duration = int(str(data.get("duration_days", "")))
        except Exception:
            raise gl.vm.UserError("REGISTRY_TERMS_MISMATCH")
        if returned_duration != requested_duration:
            raise gl.vm.UserError("REGISTRY_TERMS_MISMATCH")
        ...
        terms = {
            ...
            "binance_settlement_symbol": symbol,
            "duration_days": str(returned_duration),
        }
    ...
    def _fetch_binance_live_ticker_price_scaled(self, symbol: str) -> int:
        normalized_symbol = str(symbol).upper()
        url = f"{BINANCE_BASE_URL}/api/v3/ticker/price?symbol={normalized_symbol}"

        def fetch_ticker_result() -> typing.Any:
            response = gl.nondet.web.get(url)
            status_code = _response_status_code(response)
            if status_code != 200:
                raise gl.vm.UserError("BINANCE_TICKER_FETCH_FAILED")
            response_text = _response_to_text(response)
            if response_text == "":
                raise gl.vm.UserError("INVALID_BINANCE_TICKER_RESPONSE")
            try:
                data = json.loads(response_text, parse_float=str)
            except Exception:
                raise gl.vm.UserError("INVALID_BINANCE_TICKER_RESPONSE")
            if not isinstance(data, dict):
                raise gl.vm.UserError("INVALID_BINANCE_TICKER_RESPONSE")
            if str(data.get("symbol", "")).upper() != normalized_symbol:
                raise gl.vm.UserError("BINANCE_TICKER_PRICE_MISMATCH")
            price_text = str(data.get("price", "")).strip()
            if price_text == "":
                raise gl.vm.UserError("BINANCE_TICKER_PRICE_MISSING")
            price_scaled = self._parse_scaled_price(price_text)
            if price_scaled <= 0:
                raise gl.vm.UserError("BINANCE_TICKER_PRICE_MISSING")
            return {
        ...
        def validator_fn(leader_result) -> bool:
            if not isinstance(leader_result, gl.vm.Return):
                return False
            try:
                validator_result = fetch_ticker_result()
                leader_data = leader_result.calldata
                if leader_data["symbol"] != validator_result["symbol"]:
                    return False
                if leader_data["source"] != validator_result["source"]:
                    return False
                ...
                if leader_price <= 0 or validator_price <= 0:
                    return False
                diff = abs(leader_price - validator_price)
                max_price = max(leader_price, validator_price)
                return (
                    diff * BPS_DENOMINATOR
                    <= max_price * LIVE_PRICE_TOLERANCE_BPS
                )
            except Exception:
                return False

        result = gl.vm.run_nondet_unsafe(fetch_ticker_result, validator_fn)
    ...
    def _fetch_binance_kline_evidence(
        ...
        normalized_symbol = str(symbol).upper()
        url = (
            f"{BINANCE_BASE_URL}/api/v3/klines?symbol={normalized_symbol}&interval={interval}"
    ...
    def _validate_settlement_date(
        ...
        latest_closed_ordinal = self._date_to_ordinal(self._latest_closed_daily_date())
        ...
        if settlement_ordinal > latest_closed_ordinal:
            raise gl.vm.UserError("DAILY_CANDLE_NOT_CLOSED")
        expected_date = self._expected_settlement_date(protection)
    ...
    def _expected_settlement_date(self, protection: typing.Any) -> str:
        ...
        next_date = self._add_days(last_settled_date, 1)
        if self._date_to_ordinal(next_date) > self._date_to_ordinal(
            str(protection["coverage_end_date"])
        ):
            return ""
        return next_date

    def _dashboard_summary_for_sender(
        self, start: u256, limit: u256, paginated: bool
    ) -> str:
        wallet = self._sender_key()
        all_ids = self._get_protection_ids_for_owner(wallet)
        selected_ids = all_ids
        if paginated:
            selected_ids = self._slice_ids(all_ids, start, limit)
        protections = []
        counts = {
            STATUS_ACTIVE: 0,
            STATUS_TRIGGERED: 0,
        ...
        for protection_id in all_ids:
            protection = self._require_protection(self._protection_key_from_string(str(protection_id)))
            status = str(protection["status"])
            if status in counts:
                counts[status] += 1
        for protection_id in selected_ids:
            protection = self._require_protection(self._protection_key_from_string(str(protection_id)))
            protections.append(self._dashboard_protection_row(protection))
        result = {
            "wallet": wallet,
            "protection_count": str(len(all_ids)),
            "active_count": str(counts[STATUS_ACTIVE]),
            "triggered_count": str(counts[STATUS_TRIGGERED]),
            "paid_count": str(counts[STATUS_PAID]),
            "expired_count": str(counts[STATUS_EXPIRED]),
            "cancelled_count": str(counts[STATUS_CANCELLED]),
            "protection_ids": all_ids,
        ...
        if paginated:
            ...
            result["returned_count"] = str(len(selected_ids))
            result["total_count"] = str(len(all_ids))
            result["has_more"] = start_int + limit_int < len(all_ids)
        return self._dumps_json(result)

    def _dashboard_protection_row(self, protection: typing.Any) -> typing.Any:
        expected_date = self._expected_settlement_date(protection)
        latest_closed_date = self._latest_closed_daily_date()
        settlement_ready = False
        if str(protection["status"]) == STATUS_ACTIVE and expected_date != "":
            settlement_ready = (
                self._date_to_ordinal(expected_date)
                <= self._date_to_ordinal(latest_closed_date)
            )
        return {
            "protection_id": str(protection["protection_id"]),
            "protected_asset": str(protection["protected_asset"]),
            "protection_type": str(protection["protection_type"]),
            "event_level": str(protection["event_level"]),
            "status": str(protection["status"]),
            "premium_paid": str(protection["premium_paid"]),
            "payout_amount": str(protection["payout_amount"]),
            "reserved_payout": str(protection["reserved_payout"]),
            "reference_price_display": str(protection["reference_price_display"]),
            "trigger_price_display": str(protection["trigger_price_display"]),
            "last_daily_low_display": str(protection["last_daily_low_display"]),
            "purchase_date": str(protection["purchase_date"]),
            "duration_days": str(protection["duration_days"]),
            "coverage_start_date": str(protection["coverage_start_date"]),
            "coverage_end_date": str(protection["coverage_end_date"]),
            "last_settled_date": str(protection["last_settled_date"]),
            "triggered_date": str(protection["triggered_date"]),
            "paid_date": str(protection["paid_date"]),
            "cancelled_date": str(protection["cancelled_date"]),
            "expected_settlement_date": expected_date,
            "settlement_ready": settlement_ready,
        }

    def _slice_ids(self, ids: typing.Any, start: u256, limit: u256) -> typing.Any:
        start_int = self._u256_to_int(start)
        limit_int = self._u256_to_int(limit)
        if limit_int == 0:
            return []
    ...
    def _latest_closed_daily_date(self) -> str:
        return self._add_days(self._today_date(), -1)
    ...
    def _terms_amount_to_u256(self, terms: typing.Any, field_name: str) -> u256:
        if field_name not in terms:
            raise gl.vm.UserError("INVALID_REGISTRY_RESPONSE")
        try:
            return u256(int(str(terms[field_name])))
        except Exception:
            raise gl.vm.UserError("INVALID_REGISTRY_RESPONSE")
    ...
    def _pool_status_fields(self) -> typing.Any:
        return {
    ...
    def _positive_u256_to_int(self, value: u256, error_code: str) -> int:
        try:
    ...
    def _u256_to_int(self, value: u256) -> int:
        try:
    ...
    def _require_settlement_caller(self, protection: typing.Any) -> None:
        sender = self._sender_key()
        if sender in [self.owner, self.settlement_operator, str(protection["owner"])]:
            return
        raise gl.vm.UserError("NOT_SETTLEMENT_OPERATOR")

    def _inactive_status_error(self, status: str) -> str:
        errors = {
            STATUS_TRIGGERED: "PROTECTION_ALREADY_TRIGGERED",
            STATUS_PAID: "PROTECTION_ALREADY_PAID",
            STATUS_EXPIRED: "PROTECTION_ALREADY_EXPIRED",
            STATUS_CANCELLED: "PROTECTION_ALREADY_CANCELLED",
        }
        return errors.get(status, "PROTECTION_NOT_ACTIVE")
    ...
    def _remove_id(self, ids: typing.Any, item_id: str) -> typing.Any:
        target = str(item_id)
        return [str(current_id) for current_id in ids if str(current_id) != target]
